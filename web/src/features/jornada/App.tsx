...
export default function App() {
  ...
  // Carrega unidades (com retentativas: o backend demora a ler os Parquet antes de escutar na 3001)
  useEffect(() => {
    let cancelled = false
      ...
      fetch(`${API_BASE}/api/units`)
        ...
        .catch(() => {
          ...
          if (attempt >= 45) {
            setApiLoading(false)
            setApiError(
              `Sem conexão com a API em ${API_BASE}. Na raiz do repositório use: npm run dev (sobe frontend e backend).`
            )
            return
          }
