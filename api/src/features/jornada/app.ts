...
/** Conexão extra só para leituras em paralelo (uma query por conexão). */
const openReadConn = () => duckDb.connect();
...
const loadParquetOnConn = (conn: duckdb.Connection, filename: string): Promise<any[]> => {
...
const loadParquet = (filename: string) => loadParquetOnConn(duckConn, filename);

/**
 * Agrega vias no DuckDB (evita `SELECT *` de dezenas de MB só para deduplicar em Node).
 */
const loadViasAggregated = (conn: duckdb.Connection): Promise<void> => {
  return new Promise((resolve, reject) => {
    const filePath = path.join(DADOS_DIR, 'tbl_vias_medicamentos.parquet');
    if (!fs.existsSync(filePath)) {
      fastify.log.warn('Arquivo não encontrado: tbl_vias_medicamentos.parquet');
      return resolve();
    }
    const p = quotePathForReadParquet(filePath);
    const sql = `
      SELECT NR_ATENDIMENTO, list(DS_MATERIAL ORDER BY DS_MATERIAL) AS materiais
      FROM (
...
const loadAuxiliaryTables = async () => {
  ...
  try {
    fastify.log.info('Carga auxiliar: Parquet em paralelo (uma conexão por ficheiro) + vias agregadas no DuckDB…');

    const [labs, meds, rx, tc, revals, metasRows] = await Promise.all([
      loadParquetOnConn(openReadConn(), 'tbl_tempos_laboratorio.parquet'),
      loadParquetOnConn(openReadConn(), 'tbl_tempos_medicacao.parquet'),
      loadParquetOnConn(openReadConn(), 'tbl_tempos_rx_e_ecg.parquet'),
      loadParquetOnConn(openReadConn(), 'tbl_tempos_tc_e_us.parquet'),
      loadParquetOnConn(openReadConn(), 'tbl_tempos_reavaliacao.parquet'),
      loadParquetOnConn(openReadConn(), 'meta_tempos.parquet'),
      loadViasAggregated(openReadConn()),
    ]);
...
export const start = async () => {
  try {
    fastify.log.info(`DuckDB (memória) + Parquet; pasta: ${DADOS_DIR}`);
